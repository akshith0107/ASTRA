...
async def get_network_graph(request: Request, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    result = {
        "nodes": [{"id": n.id, "type": n.node_type, "value": n.node_value, "risk_score": n.risk_score} for n in nodes],
        "edges": [{"id": e.id, "source": e.source_node_id, "target": e.target_node_id, "type": e.relationship_type, "confidence": 1.0} for e in edges]
    }
